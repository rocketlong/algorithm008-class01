...
public class Minesweeper {
    ...
    class Solution {
        ...
        public char[][] updateBoard(char[][] board, int[] click) {
            if (board == null || board.length == 0 || click == null) return board;
            m = board.length;
            n = board[0].length;
            int row = click[0];
            int col = click[1];
        ...
        private void dfs(char[][] board, int row, int col) {
            ...
            int num = getNumsOfBombs(board, row, col);
            if (num == 0) {
                board[row][col] = 'B';
                for (int i = -1; i < 2; i++) {
                    for (int j = -1; j < 2; j++) {
        ...
        private int getNumsOfBombs(char[][] board, int row, int col) {
            int num = 0;
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    if (i == 0 && j == 0) continue;
                    if (row + i < 0 || row + i >= m || col + j < 0 || col + j >= n) continue;
                    if (board[row + i][col + j] == 'M') num++;
                }
            }
            return num;
        }
